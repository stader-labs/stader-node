...
func NewConfig() *GraffitiWallWriterConfig {
	return &GraffitiWallWriterConfig{
		...
		InputURL: config.Parameter{
			ID:                   "inputUrl",
			Name:                 "Input URL",
			Description:          "URL or filepath for the input JSON file that contains the graffiti image to write to the wall. By default, this is the Stader logo.\n\nSee https://gist.github.com/RomiRand/dfa1b5286af3e926deff0be2746db2df for info on making your own images.\n\nNOTE: for local files, you must manually put the file into the `addons/gww` folder of your `stader` directory, and then enter the name of it as `/gww/<filename>` here.",
		...
		AdditionalFlags: config.Parameter{
			...
			Description:          "Additional custom command line flags you want to pass to the addon, to take advantage of other settings that the Stadernode's configuration doesn't cover.",
			Type:                 config.ParameterType_String,
