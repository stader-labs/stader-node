...
// Make a node deposit
func (c *Client) NodeDeposit(amountWei *big.Int, salt *big.Int, submit bool) (api.NodeDepositResponse, error) {
	responseBytes, err := c.callAPI(fmt.Sprintf("node deposit %s %s %s %s %t", amountWei.String(), salt.String(), submit))
	if err != nil {
