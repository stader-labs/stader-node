...
if type sudo > /dev/null 2>&1; then
    ...
    fail "Please make sure a privilege escalation command such as \"sudo\" is installed and available before installing Stader CLI."
fi
...
{ mkdir -p "$DATA_PATH/validators" || fail "Could not create theStader user data directory."; } >&2
{ mkdir -p "$RP_PATH/runtime" || fail "Could not create theStader runtime directory."; } >&2
{ mkdir -p "$DATA_PATH/secrets" || fail "Could not create theStader secrets directory."; } >&2
{ mkdir -p "$DATA_PATH/rewards-trees" || fail "Could not create theStader rewards trees directory."; } >&2
...
# Copy package files
progress 8 "Copying package files toStader user data directory..."
{ cp -r "$PACKAGE_FILES_PATH/addons" "$RP_PATH" || fail "Could not copy addons folder to theStader user data directory."; } >&2
{ cp -r -n "$PACKAGE_FILES_PATH/override" "$RP_PATH" || rsync -r --ignore-existing "$PACKAGE_FILES_PATH/override" "$RP_PATH" || fail "Could not copy new override files to theStader user data directory."; } >&2
{ cp -r "$PACKAGE_FILES_PATH/scripts" "$RP_PATH" || fail "Could not copy scripts folder to theStader user data directory."; } >&2
{ cp -r "$PACKAGE_FILES_PATH/templates" "$RP_PATH" || fail "Could not copy templates folder to theStader user data directory."; } >&2
{ cp "$PACKAGE_FILES_PATH/grafana-prometheus-datasource.yml" "$PACKAGE_FILES_PATH/prometheus.tmpl" "$RP_PATH" || fail "Could not copy base files to theStader user data directory."; } >&2
