...
func voteOnProposal(c *cli.Context) error {
	...
	allMembers, err := staderClient.TNDAOMembers()
	if err != nil {
		return err
	}
	...
	canVote, err := staderClient.CanVoteOnTNDAOProposal(selectedProposal.ID)
	if err != nil {
		return err
	}
	if !canVote.CanVote {
	...
	// Assign max fees
	err = gas.AssignMaxFeeAndLimit(canVote.GasInfo, staderClient, c.Bool("yes"))
	if err != nil {
		return err
	}
	...
	// Vote on proposal
	response, err := staderClient.VoteOnTNDAOProposal(selectedProposal.ID, support)
	if err != nil {
		return err
	}
