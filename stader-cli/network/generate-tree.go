...
func generateRewardsTree(c *cli.Context) error {
	...
	canResponse, err := staderClient.CanGenerateRewardsTree(index)
	if err != nil {
		return err
	}
	...
	_, err = staderClient.GenerateRewardsTree(index)
	if err != nil {
		return err
	}
	...
	if c.Bool("yes") || cliutils.Confirm("Would you like to restart the watchtower container now, so it starts generating the file immediately?") {
		...
		response, err := staderClient.RestartContainer(container)
		if err != nil {
