...
func getActiveDAOProposals(c *cli.Context) error {
	...
	proposalsResponse, err := staderClient.GetActiveDAOProposals()
	if err != nil {
		return err
	}
	...
	if proposalsResponse.VotingDelegate == blankAddress {
		fmt.Println("The node does not currently have a voting delegate set, and will not be able to vote on Stader governance proposals.")
	} else {
		fmt.Printf("The node has a voting delegate of %s%s%s which can represent it when voting on Stader governance proposals.\n", colorBlue, proposalsResponse.VotingDelegate.Hex(), colorReset)
	}
	...
	if len(proposalsResponse.ActiveSnapshotProposals) == 0 {
		fmt.Print("Stader has no governance proposals being voted on.\n")
	} else {
		fmt.Printf("Stader has %d governance proposal(s) being voted on. You have voted on %d of those.\n", len(proposalsResponse.ActiveSnapshotProposals), voteCount)
	}
