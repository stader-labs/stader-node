...
func createNewUserExecutionContent() tview.Primitive {
	...
	rpClientCheckbox := tview.NewCheckbox().
		SetLabel("I want Stader to manage my Execution client")
	rpClientCheckbox.SetFocusFunc(func() {
		layout.descriptionBox.SetText("Choose this if you don't already have your own Execution client running, and you want Stader to create and manage one for you.")
	})
	form.AddFormItem(rpClientCheckbox)

	externalClientCheckbox := tview.NewCheckbox().
		SetLabel("I want to use an existing Execution client that I manage outside of Stader")
	externalClientCheckbox.SetFocusFunc(func() {
		layout.descriptionBox.SetText("Choose this if you have your own Execution client installed and running on this machine or another one, and you want Stader to link to that instead of running its own client.")
	})
