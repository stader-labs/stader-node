...
func createExternalDoppelgangerStep(wiz *wizard, currentStep int, totalSteps int) *choiceWizardStep {
	...
	helperText := "Your client supports Doppelganger Protection. This feature can prevent your validators from being slashed (penalized for a lot of ETH and removed from the Beacon Chain) if you accidentally run your validator keys on multiple machines at the same time.\n\nIf enabled, whenever your validator client restarts, it will intentionally miss 2-3 attestations (for each validator). If all of them are missed successfully, you can be confident that you are safe to start attesting.\n\nWould you like to enable Doppelganger Protection?"
