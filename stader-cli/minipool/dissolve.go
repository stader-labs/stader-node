...
func dissolveMinipools(c *cli.Context) error {
	...
	for _, minipool := range selectedMinipools {
		canResponse, err := staderClient.CanDissolveMinipool(minipool.Address)
		if err != nil {
	...
	for _, minipool := range selectedMinipools {
		response, err := staderClient.DissolveMinipool(minipool.Address)
		if err != nil {
		...
		closeResponse, err := staderClient.CloseMinipool(minipool.Address)
		if err != nil {
		...
		cliutils.PrintTransactionHash(staderClient, closeResponse.TxHash)
		if _, err = staderClient.WaitForTransaction(closeResponse.TxHash); err != nil {
			fmt.Printf("Could not close minipool %s: %s.\n", minipool.Address.Hex(), err)
		} else {
