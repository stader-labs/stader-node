...
func nodeClaimRewards(c *cli.Context) error {
	...
	rewardsInfoResponse, err := staderClient.GetRewardsInfo()
	if err != nil {
	...
	if restakeAmountWei == nil {
		canClaim, err := staderClient.CanNodeClaimRewards(indices)
		if err != nil {
			return err
		}

		// Assign max fees
		err = gas.AssignMaxFeeAndLimit(canClaim.GasInfo, staderClient, c.Bool("yes"))
		if err != nil {
			return err
		}
	} else {
		canClaim, err := staderClient.CanNodeClaimAndStakeRewards(indices, restakeAmountWei)
		if err != nil {
			return err
		}
	...
	if restakeAmountWei == nil {
		response, err := staderClient.NodeClaimRewards(indices)
		if err != nil {
			return err
		}
		txHash = response.TxHash
	} else {
		response, err := staderClient.NodeClaimAndStakeRewards(indices, restakeAmountWei)
		if err != nil {
			return err
		}
		txHash = response.TxHash
	...
	cliutils.PrintTransactionHash(staderClient, txHash)
	if _, err = staderClient.WaitForTransaction(txHash); err != nil {
		return err
	}
