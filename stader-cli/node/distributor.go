...
func initializeFeeDistributor(c *cli.Context) error {
	...
	gasResponse, err := staderClient.GetInitializeFeeDistributorGas()
	if err != nil {
		return err
	}

	// Assign max fees
	err = gas.AssignMaxFeeAndLimit(gasResponse.GasInfo, staderClient, c.Bool("yes"))
	if err != nil {
		return err
	}
	...
	// Initialize it
	response, err := staderClient.InitializeFeeDistributor()
	if err != nil {
		return err
	}
...
func distribute(c *cli.Context) error {
	...
	// Get the gas estimate
	canDistributeResponse, err := staderClient.CanDistribute()
	if err != nil {
		return err
	}
	...
	// Assign max fees
	err = gas.AssignMaxFeeAndLimit(canDistributeResponse.GasInfo, staderClient, c.Bool("yes"))
	if err != nil {
		return err
	}
	...
	// Distribute
	response, err := staderClient.Distribute()
	if err != nil {
		return err
	}
