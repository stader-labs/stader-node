...
func testRecovery(c *cli.Context) error {
	...
	if !skipValidatorKeyRecovery {
		customKeyPasswordFile, err := promptForCustomKeyPasswords(staderClient, cfg, true)
		if err != nil {
			return err
		}
	...
	if addressString != "" {
		...
		// Test recover wallet
		response, err := staderClient.TestSearchAndRecoverWallet(mnemonic, address, skipValidatorKeyRecovery)
		if err != nil {
			return err
		}
		...
		// Test recover wallet
		response, err := staderClient.TestRecoverWallet(mnemonic, skipValidatorKeyRecovery, derivationPath, walletIndex)
		if err != nil {
			return err
		}
